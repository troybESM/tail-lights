...
def get_winner(event, context):
    ...
    if len(number_response) > 0:
        ...
        winner_item = [
            res for res in win_result if res['sk'].startswith('USER#')]
    else:
...
def show_entrants(event, context):
    ...
    if len(users) > 0:
        ...
        page = build_html_page(content=f"Entrants are: <ul>{
                               ''.join(user_names)}<ul>")
...
def show_unique_entrants(event, context):
    ...
    if len(users) > 0:
        ...
        counter_content = [f"<li>{item[0]}: {
            item[1]}</li>" for item in counter]
        page = build_html_page(content=f"Entrants are: <ul>{
                               ''.join(counter_content)}<ul>")
